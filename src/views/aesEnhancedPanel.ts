...
import { addAesKeyIdentifier, getAesKeyIdentifiers } from '../storage/keyStore';
...
export class AESEnhancedPanel {
    ...
    private readonly _onKeysChanged?: () => void;
    private _disposables: vscode.Disposable[] = [];

    private constructor(panel: vscode.WebviewPanel, context: vscode.ExtensionContext, onKeysChanged?: () => void) {
        this._panel = panel;
            ...
            (message) => {
                switch (message.command) {
                    ...
                    case 'decrypt':
                        ...
                        return;
                    case 'saveKey':
                        void this._saveKeyIdentifier(message.keyIdentifier, message.key);
                        return;
                    case 'refreshKeys':
                        void this._initHtml();
                        return;
    ...
    public static render(context: vscode.ExtensionContext, onKeysChanged?: () => void): void {
        ...
        AESEnhancedPanel.currentPanel = new AESEnhancedPanel(panel, context, onKeysChanged);
    }
    ...
    private async _saveKeyIdentifier(keyIdentifier: string, key: string): Promise<void> {
        const name = (keyIdentifier ?? '').trim();
        if (!name || !key) {
            this._panel.webview.postMessage({
                command: 'error',
                message: 'A Key Identifier name and a key are both required to save.',
            });
            return;
        }
        try {
            await addAesKeyIdentifier(this._context, name, key);
            this._panel.webview.postMessage({ command: 'keySaved', keyIdentifier: name, key });
            this._onKeysChanged?.();
        } catch (error) {
            const detail = error instanceof Error ? error.message : String(error);
            this._panel.webview.postMessage({ command: 'error', message: `Failed to save key: ${detail}` });
        }
    }
...
<head>
    ...
    <style>
        ...
        .icon-btn:hover { background-color: var(--vscode-button-secondaryHoverBackground); }
        ...
        .modal-title { font-weight: bold; font-size: 1.05em; margin-bottom: 14px; }
        .modal input[type="text"] { margin-bottom: 6px; }
        .modal-error {
            color: var(--vscode-errorForeground);
            font-size: 0.85em;
            min-height: 1.1em;
...
<body>
    <div class="container">
        <div class="header-row">
            ...
            <button type="button" id="refreshKeysBtn" class="icon-btn" title="Refresh keys">
            ...
            <div class="key-row">
                <div class="key-input">
                    ...
                    <div class="key-input-wrapper">
                        <input type="text" id="key" placeholder="Enter your encryption key" />
                        ...
                        <button type="button" id="saveKeyBtn" class="toggle-visibility" title="Save key as KeyIdentifier" style="display: none;">
                            <svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M17 3H5c-1.11 0-2 .9-2 2v14c0 1.1.89 2 2 2h14c1.1 0 2-.9 2-2V7l-4-4zm2 16H5V5h11.17L19 7.83V19zm-7-7c-1.66 0-3 1.34-3 3s1.34 3 3 3 3-1.34 3-3-1.34-3-3-3zM6 6h9v4H6V6z"/></svg>
                        </button>
                    </div>
                </div>
    ...
    <div class="modal-overlay" id="saveKeyModalOverlay">
        <div class="modal">
            <div class="modal-title">Save Encryption Key</div>
            <input type="text" id="saveKeyName" placeholder="Key Identifier" autocomplete="off" />
            <div class="modal-error" id="saveKeyError"></div>
            <div class="modal-actions">
                <button type="button" id="saveKeyConfirmBtn">Save</button>
                <button type="button" id="saveKeyCloseBtn" class="secondary">Close</button>
            </div>
        </div>
    </div>
    ...
    <script nonce="${nonce}">
        ...
        const saveKeyModal = document.getElementById('saveKeyModalOverlay');
        const saveKeyName = document.getElementById('saveKeyName');
        const saveKeyError = document.getElementById('saveKeyError');
        ...
        // The save button only appears for a custom (non-KeyIdentifier) key that has been typed.
        function updateSaveButtonVisibility() {
            const hasCustomKey = !isKeyIdentifierSelected && currentKey().trim().length > 0;
            saveKeyBtn.style.display = hasCustomKey ? 'flex' : 'none';
        }
        ...
        function openSaveModal() {
            saveKeyError.textContent = '';
            saveKeyName.value = '';
            saveKeyModal.style.display = 'flex';
            saveKeyName.focus();
        }

        function closeSaveModal() {
            saveKeyModal.style.display = 'none';
        }
        ...
        function submitSaveKey() {
            ...
            const key = currentKey();
            if (!name) {
                saveKeyError.textContent = 'Please enter a Key Identifier.';
                return;
            }
            if (!key) {
                saveKeyError.textContent = 'There is no key to save.';
                return;
            }
            vscode.postMessage({ command: 'saveKey', keyIdentifier: name, key });
        }

        saveKeyBtn.addEventListener('click', openSaveModal);
        document.getElementById('saveKeyConfirmBtn').addEventListener('click', submitSaveKey);
        document.getElementById('saveKeyCloseBtn').addEventListener('click', closeSaveModal);
        ...
        saveKeyModal.addEventListener('click', (e) => {
            if (e.target === saveKeyModal) {
                closeSaveModal();
            }
        });

        function upsertKeyIdentifierOption(name, key) {
            let option = Array.from(keyIdentifierSelect.options).find((o) => o.textContent === name);
            if (!option) {
                option = document.createElement('option');
                keyIdentifierSelect.appendChild(option);
            }
            option.value = key;
            option.textContent = name;
            keyIdentifierSelect.value = key;
            applyKeyIdentifierSelection(key);
        }
