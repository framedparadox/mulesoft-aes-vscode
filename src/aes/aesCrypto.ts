...
/**
 * MuleSoft-compatible symmetric encryption / decryption.
 *
 * MuleSoft's secure configuration properties tool can encrypt with several
 * algorithms (AES, Blowfish, DES, DESede, RC2, RCA), cipher modes (CBC, CFB,
 * ECB, OFB) and an optional random IV, wrapping the ciphertext in the
 * `![base64]` format.
 *
 * The defaults — AES / CBC with the IV derived from the first 16 characters of
 * the key and no random IV — reproduce the original MuleSoft AES behaviour: a
 * 16-character key selects AES-128, 24 selects AES-192 and 32 selects AES-256.
 *
 * When random IVs are enabled the generated IV is prepended to the ciphertext
 * before base64 encoding (and read back from the front on decryption), matching
 * the random-IV behaviour of the secure properties generator.
 */
